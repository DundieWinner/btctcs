...
> = ({ announcements, priceHistory }) => {
  ...
  const calculateSatsPerShare = (
    btcHeld: number,
    sharesOutstanding: number,
  ) => {
    const totalSats = btcHeld * 100000000;
    return Math.round(
      sharesOutstanding > 0 ? totalSats / sharesOutstanding : 0,
    );
  };
  ...
  const calculateAdjustedMNAVForDate = (priceRecord: PriceHistoryRecord) => {
    ...
    const bitcoinValueInGBP =
      announcementData.btcHeld * priceRecord.bitcoinPrice;
